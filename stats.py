...
class RequestStats(object):
    ...
    def get_my_entry(self,name):
        entry = self.my_entries.get(name)
        if not entry:
            entry = MyEntry(name)
            self.my_entries[name] = entry
...
def on_job_finish(name,jobtime):
    global_stats.get_my_entry(name).log(jobtime)
...
events.job_finish += on_job_finish
